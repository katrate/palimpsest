...
try {
  ...
  if ($NeedsPathAdd) {
    ...
    Write-Host "  NOTE: Added ${InstallDir} to your user PATH." -ForegroundColor Cyan
    Write-Host "  You may need to restart your terminal for it to take effect." -ForegroundColor Cyan
  }

  Write-Host "OK palin installed successfully at ${InstallDir}\${Binary}.exe" -ForegroundColor Green
  Write-Host ""
